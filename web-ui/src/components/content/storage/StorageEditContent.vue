<template>
  <div>
    <form novalidate @submit.prevent="validateStorage">
      ...
      <md-snackbar
        :md-active.sync="storageUpdated"
      >
        {{messages.STORAGE_UPDATED}}
      </md-snackbar>
    </form>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      updateStorage() {
        ...
        this.$http.put(`${Url.STORAGE}/${this.$route.params.id}`, JSON.stringify(form), {
          headers: {
    ...
    mounted: function() {
      ...
      this.$http.get(`${Url.STORAGE}/${this.$route.params.id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.accessToken}`
        }
      })
