<template>
  <div>
    <form novalidate @submit.prevent="validateProductOwner">
      ...
      <md-snackbar
        :md-active.sync="productOwnerSaved"
      >
        {{messages.PRODUCT_OWNER_SAVED}}
      </md-snackbar>
    </form>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      saveProductOwner() {
        ...
        this.$http.post(Url.PRODUCT_OWNER, JSON.stringify(form), {
          headers: {
            Authorization: `Bearer ${localStorage.accessToken}`
          }
        })
          .then(() => {
            this.productOwnerSaved = true;
            this.sending = false;
            this.cleanForm();
            this.hasError = false;
          }, response => {
